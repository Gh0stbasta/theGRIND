...
const rl = readline.createInterface({ // We need to create an interface for readling to tell him where the cool stuff happens
...
const answers = [
  ...
  (age) =>
    `\n\nOhhh.... ${age.trim()} is really old. Which sport are you doing to keep fit?`, // I hate the template stringt, but this has style!
  (sport) =>
...
const askQuestion = () => { // setting up the function to ask some questions
    if (index < questions.length) { // yep, if we have some questions left, we are going forward through this
        ...
        rl.close(); // yup, when were done were going home :)
    }
};
