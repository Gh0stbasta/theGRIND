...
const rl = readline.createInterface({
  // We need to create an interface for readline to tell him where the cool stuff happens
...
const askQuestion = () => {
  // setting up the function to ask some questions
  if (index < questions.length) {
    // yep, if we have some questions left, we are going forward through this
    rl.question(questions[index], (answer) => {
      // again pretty cool stuff, since the .question method is just a method that writes the first argument to the output and waits for the response
